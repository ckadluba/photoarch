...
class TestCaptionComparer(unittest.TestCase):
    ...
    def test_captions_are_different_identical_captions(self):
        ...
        score = calculate_caption_difference("a dog playing in the park", "a dog playing in the park", self.context)
        self.assertLess(score, 0.05)

    def test_captions_are_different_similar_captions(self):
        """Similar captions should have a low difference score."""
        score = calculate_caption_difference("a dog playing in the snow", "a puppy running in snow", self.context)
        self.assertLess(score, 0.5)

    def test_captions_are_different_different_captions(self):
    ...
    def test_captions_are_different_empty_captions(self):
        """Empty captions should return 0.0 (cannot determine difference)."""
        self.assertEqual(calculate_caption_difference("", "", self.context), 0.0)
        self.assertEqual(calculate_caption_difference("some caption", "", self.context), 0.0)
        self.assertEqual(calculate_caption_difference("", "some caption", self.context), 0.0)

    def test_captions_are_different_returns_float_in_range(self):
        """captions_are_different should return a float between 0.0 and 1.0."""
        score = calculate_caption_difference("a dog in the park", "a cat on a sofa", self.context)
